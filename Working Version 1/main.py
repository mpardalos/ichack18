...
with open('raw.csv', 'r') as csvfile:
    ...
    raw_output = [[((movies.index(x[2]) + 1) / len(movies))] for x in raw_data]
    print(raw_output)
    ...
    nn.train(training_set_raw_inputs, training_set_raw_outputs, 100000)

    output = nn.forward(training_set_raw_inputs)

    print([movies[int(round(x[0] * len(movies)) - 1)] for x in output])
    print([movies[int(round(x[0] * len(movies)) - 1)] for x in training_set_raw_outputs])
